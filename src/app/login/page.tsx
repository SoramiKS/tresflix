...
export default function LoginPage() {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    const url = isRegister ? "/auth/signup" : "/auth/signin";
    ...
    try {
      ...
      localStorage.setItem("userSession", JSON.stringify({
        ...
        name: isRegister ? name : data.name || email.split("@")[0]
            ...
            router.refresh(); // Biar layout ke-refresh
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-[#2f4156] text-white px-4">
      ...
      <div className="w-full h-[600px] bg-[url('/login-bg.svg')] bg-no-repeat bg-contain bg-center flex items-center justify-center m-2">
        <div className="bg-white rounded-2xl shadow-2xl p-6 sm:p-10 w-full max-w-xs text-center text-black transform transition duration-300 hover:-translate-y-2 hover:shadow-[0_20px_50px_rgba(0,0,0,0.3)]">
          {isRegister && (
            <>
